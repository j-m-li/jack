...
#ifndef JACK_CharPainter_H
#define JACK_CharPainter_H
var CharPainter__new();
var CharPainter__paint(var __this, var grid);
var CharPainter__getWidth(var __this);
var CharPainter__getHeight(var __this);
var CharPainter__dispose(var __this);
#endif
...
#ifndef JACK_Debug_H
#define JACK_Debug_H
var Debug__printLiveCount(var uni);
#endif
...
#define JACK_Cell_H
var Cell__new();
var Cell__update(var __this, var newState);
var Cell__next(var __this, var aliveNeighborsCount);
var Cell__nextWhenAlive(var __this, var aliveNeighborsCount);
var Cell__nextWhenDead(var __this, var aliveNeighborsCount);
var Cell__dispose(var __this);
#endif
...
#ifndef JACK_Utils_H
#define JACK_Utils_H
var Utils__modulo(var numerator, var denominator);
var Utils__assertInt(var value, var expected);
#endif
...
#ifndef JACK_Generator_H
#define JACK_Generator_H
var Generator__emptyGrid(var w, var h);
var Generator__seededGridOne(var w, var h);
var Generator__seededGridTwo(var w, var h);
#endif
...
#ifndef JACK_Universe_H
#define JACK_Universe_H
var Universe__new(var w, var h);
var Universe__next(var __this);
var Universe__updateWorld(var __this, var i, var grid, var nextGrid);
var Universe__bounds(var __this, var i);
var Universe__getSize(var __this);
var Universe__getGrid(var __this);
var Universe__dispose(var __this);
#endif
...
#ifdef JACK_IMPLEMENTATION
#define DEAD__ (__this+0)
#define ALIVE__ (__this+1)
#define width__ (__this+2)
#define height__ (__this+3)
#define size__ (__this+4)
var CharPainter__new() {
	var __this;
	...
	__poke(width__, 64);
	__poke(height__, 23);
	__poke(size__, __peek(width__)*__peek(height__));
	return __this;
}
var CharPainter__paint(var __this, var grid) {
	var currentChar = 0;
	...
	while (-1==((((i)<(__peek(size__)))?-1:0))) {
		if (0!=((((__peek(grid+i))==(0))?-1:0))) {
			currentChar = __peek(DEAD__);
		} else {
			currentChar = __peek(ALIVE__);
		}
		Output__printChar(currentChar);
		i = i+1;
	}
	return 0;
}
var CharPainter__getWidth(var __this) {
	return __peek(width__);
}
var CharPainter__getHeight(var __this) {
	return __peek(height__);
}
var CharPainter__dispose(var __this) {
	Memory__deAlloc(__this);
	return 0;
...
var Debug__printLiveCount(var uni) {
	...
	i = 0;
	count = 0;
	size = Universe__getSize(uni);
	grid = Universe__getGrid(uni);
	while (-1==((((i)<(size))?-1:0))) {
		count = count+__peek(grid+i);
		i = i+1;
	}
	Output__printInt(count);
	return 0;
}
#endif

#ifdef JACK_IMPLEMENTATION
#define state__ (__this+0)
#define DEAD__ (__this+1)
#define ALIVE__ (__this+2)
var Cell__new() {
	var __this;
...
var Cell__update(var __this, var newState) {
	__poke(state__, newState);
	return 0;
}
var Cell__next(var __this, var aliveNeighborsCount) {
	if (0!=((((__peek(state__))==(__peek(ALIVE__)))?-1:0))) {
		return Cell__nextWhenAlive(__this, aliveNeighborsCount);
	}

	return Cell__nextWhenDead(__this, aliveNeighborsCount);
}
var Cell__nextWhenAlive(var __this, var aliveNeighborsCount) {
	if (0!=((((aliveNeighborsCount)<(2))?-1:0))) {
		return __peek(DEAD__);
	}

	if (0!=((((aliveNeighborsCount)>(3))?-1:0))) {
		return __peek(DEAD__);
	}

	return __peek(ALIVE__);
}
var Cell__nextWhenDead(var __this, var aliveNeighborsCount) {
	if (0!=((((aliveNeighborsCount)==(3))?-1:0))) {
		return __peek(ALIVE__);
...
#ifdef JACK_IMPLEMENTATION
var Utils___str0[] = {69,114,114,111,114,32,97,115,115,101,114,116,105,110,103,32,105,110,116,46,32,69,120,112,101,99,116,101,100,32,0};
var Utils___str1[] = {32,98,117,116,32,103,111,116,32,0};
var Utils__modulo(var numerator, var denominator) {
	var remainder = 0;
	var divided = 0;
	var multiplied = 0;
	divided = Math__divide(numerator, denominator);
	multiplied = Math__multiply(divided, denominator);
	remainder = numerator-multiplied;
	return remainder;
}
var Utils__assertInt(var value, var expected) {
	if (0!=(~((((value)==(expected))?-1:0)))) {
		Output__moveCursor(0, 0);
		Output__printString(Memory__getString(Utils___str0));
		Output__printInt(expected);
		Output__printString(Memory__getString(Utils___str1));
		Output__printInt(value);
		Sys__error(2);
	}
...
var Generator__emptyGrid(var w, var h) {
	...
	size = w*h;
	i = 0;
	Utils__assertInt(size, 1472);
	grid = Array__new(size);
	while (-1==((((i)<(size))?-1:0))) {
		__poke(grid+i, DEAD);
		i = i+1;
	}
...
var Generator__seededGridOne(var w, var h) {
	...
	var ALIVE = 0;
	ALIVE = 1;
	grid = Generator__emptyGrid(w, h);
	__poke(grid+4*w+30, ALIVE);
	__poke(grid+4*w+31, ALIVE);
	__poke(grid+4*w+32, ALIVE);
	__poke(grid+5*w+29, ALIVE);
	__poke(grid+5*w+31, ALIVE);
	__poke(grid+5*w+33, ALIVE);
	__poke(grid+6*w+31, ALIVE);
	__poke(grid+7*w+31, ALIVE);
	__poke(grid+8*w+31, ALIVE);
	__poke(grid+9*w+31, ALIVE);
	__poke(grid+10*w+31, ALIVE);
	__poke(grid+11*w+30, ALIVE);
	__poke(grid+11*w+31, ALIVE);
	__poke(grid+11*w+32, ALIVE);
	__poke(grid+12*w+30, ALIVE);
	__poke(grid+12*w+31, ALIVE);
	__poke(grid+12*w+32, ALIVE);
	__poke(grid+13*w+30, ALIVE);
	__poke(grid+13*w+31, ALIVE);
	__poke(grid+13*w+32, ALIVE);
	__poke(grid+0, ALIVE);
	return grid;
}
var Generator__seededGridTwo(var w, var h) {
...
#ifdef JACK_IMPLEMENTATION
#define gridA__ (__this+0)
#define gridB__ (__this+1)
#define currentGrid__ (__this+2)
#define currentCell__ (__this+3)
#define neighbors__ (__this+4)
#define width__ (__this+5)
#define height__ (__this+6)
#define size__ (__this+7)
#define neighborSize__ (__this+8)
#define DEAD__ (__this+9)
#define ALIVE__ (__this+10)
var Universe__new(var w, var h) {
	var __this;
	__this = Memory__alloc(11);
	Utils__assertInt(w, 64);
	Utils__assertInt(h, 23);
	__poke(DEAD__, 0);
	__poke(ALIVE__, 1);
	__poke(width__, w);
	__poke(height__, h);
	__poke(size__, __peek(width__)*__peek(height__));
	__poke(currentCell__, Cell__new());
	__poke(gridA__, Generator__seededGridTwo(w, h));
	__poke(gridB__, Generator__emptyGrid(w, h));
	__poke(currentGrid__, __peek(gridA__));
	return __this;
}
var Universe__next(var __this) {
	var grid = 0;
	...
	var i = 0;
	i = 0;
	if (0!=((((__peek(currentGrid__))==(__peek(gridA__)))?-1:0))) {
		grid = __peek(gridA__);
		nextGrid = __peek(gridB__);
	} else {
		grid = __peek(gridB__);
		nextGrid = __peek(gridA__);
	}
	while (-1==((((i)<(__peek(size__)))?-1:0))) {
		Universe__updateWorld(__this, i, grid, nextGrid);
		i = i+1;
	}
	if (0!=((((__peek(currentGrid__))==(__peek(gridA__)))?-1:0))) {
		__poke(currentGrid__, __peek(gridB__));
	} else {
		__poke(currentGrid__, __peek(gridA__));
	}
	return 0;
...
var Universe__updateWorld(var __this, var i, var grid, var nextGrid) {
	...
	t = Universe__bounds(__this, i-__peek(width__));
	tl = Universe__bounds(__this, t-1);
	tr = Universe__bounds(__this, t+1);
	l = Universe__bounds(__this, i-1);
	r = Universe__bounds(__this, i+1);
	b = Universe__bounds(__this, i+__peek(width__));
	bl = Universe__bounds(__this, b-1);
	br = Universe__bounds(__this, b+1);
	Cell__update(__peek(currentCell__), __peek(grid+i));
	aliveNeighborCount = __peek(grid+tl)+__peek(grid+t)+__peek(grid+tr)+__peek(grid+l)+__peek(grid+r)+__peek(grid+bl)+__peek(grid+b)+__peek(grid+br);
	__poke(nextGrid+i, Cell__next(__peek(currentCell__), aliveNeighborCount));
	return 0;
}
var Universe__bounds(var __this, var i) {
	if (0!=((((i)<(0))?-1:0))) {
		return __peek(size__)-i;
	}

	if (0!=((((i)>((__peek(size__)-1)))?-1:0))) {
		return i-__peek(size__);
	}
...
var Universe__getGrid(var __this) {
	return __peek(currentGrid__);
}
var Universe__dispose(var __this) {
	Cell__dispose(__peek(currentCell__));
	Array__dispose(__peek(gridA__));
	Array__dispose(__peek(gridB__));
	Memory__deAlloc(__this);
	return 0;
}
#undef gridA__
...
var Main__main() {
	...
	var painter = 0;
	var key = 0;
	painter = CharPainter__new();
	uni = Universe__new(CharPainter__getWidth(painter), CharPainter__getHeight(painter));
	CharPainter__paint(painter, Universe__getGrid(uni));
	while (-1==(-1)) {
		Sys__wait(100);
		Universe__next(uni);
		CharPainter__paint(painter, Universe__getGrid(uni));
		...
		if (0!=((((key)==(81))?-1:0))) {
			Universe__dispose(uni);
			CharPainter__dispose(painter);
			Sys__halt();
		}
