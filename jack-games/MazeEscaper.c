...
#ifndef JACK_PlayHelper_H
#define JACK_PlayHelper_H
var PlayHelper__drawPlayHelper();
var PlayHelper__drawUpArrow(var base);
var PlayHelper__drawDownArrow(var base);
var PlayHelper__drawRightArrow(var base);
var PlayHelper__drawLeftArrow(var base);
var PlayHelper__drawRightLine();
#endif
...
#ifndef JACK_PlayerController_H
#define JACK_PlayerController_H
var PlayerController__new(var newPlayer, var newMaze, var xOrigin, var yOrigin, var bw, var bh, var xc, var yc, var pad);
var PlayerController__loop(var __this);
var PlayerController__canPlayerGo(var __this, var direction);
#endif
...
#ifndef JACK_PseudoRand_H
#define JACK_PseudoRand_H
var PseudoRand__def(var x);
var PseudoRand__next(var __this, var mod);
var PseudoRand__modula(var __this, var number, var mod);
#endif
...
#ifndef JACK_MazeDrawer_H
#define JACK_MazeDrawer_H
var MazeDrawer__drawMaze(var maze, var originX, var originY, var blockWidth, var blockHeight, var xCount, var yCount);
var MazeDrawer__drawEastWall(var x, var y, var originX, var originY, var bw, var bh);
var MazeDrawer__drawSouthWall(var x, var y, var originX, var originY, var bw, var bh);
#endif
...
#ifndef JACK_MazeGenerator_H
#define JACK_MazeGenerator_H
var MazeGenerator__generate(var grid, var rng, var mazeWidth, var mazeHeight);
var MazeGenerator__carvePassageFrom(var cx, var cy, var grid, var rng, var mazeWidth, var mazeHeight);
#endif
...
var Player__new(var xVal, var yVal);
var Player__getX(var __this);
var Player__getY(var __this);
var Player__go(var __this, var direction);
#endif
...
var ExtendedMath__pow(var a, var b);
#endif
...
#define JACK_Main_H
var Main__main();
var Main__drawEndPoint(var baseX, var baseY, var bw, var bh, var xc, var yc, var rad);
var Main__getFirstKeyPressed();
#endif
...
#ifdef JACK_IMPLEMENTATION
var PlayHelper___str0[] = {32,32,75,69,89,83,0};
var PlayHelper___str1[] = {32,32,85,80,0};
var PlayHelper___str2[] = {32,32,68,79,87,78,0};
var PlayHelper___str3[] = {32,32,76,69,70,84,0};
var PlayHelper___str4[] = {32,32,82,73,71,72,84,0};
var PlayHelper___str5[] = {104,98,117,115,117,108,0};
var PlayHelper___str6[] = {64,71,73,84,72,85,66,0};
var PlayHelper___str7[] = {80,114,101,115,115,32,97,32,114,97,110,100,111,109,32,107,101,121,32,116,111,32,115,116,97,114,116,0};
var PlayHelper__drawPlayHelper() {
	var screenBase = 0;
	var increment = 0;
	screenBase = 16384+(4*352);
	increment = 352;
	Output__moveCursor(2, 1);
	Output__printString(Memory__getString(PlayHelper___str0));
	PlayHelper__drawUpArrow(screenBase);
	Output__moveCursor(4, 1);
	Output__printString(Memory__getString(PlayHelper___str1));
	screenBase = screenBase+(2*increment);
	PlayHelper__drawDownArrow(screenBase);
	Output__moveCursor(6, 1);
	Output__printString(Memory__getString(PlayHelper___str2));
	screenBase = screenBase+(2*increment);
	PlayHelper__drawLeftArrow(screenBase);
	Output__moveCursor(8, 1);
	Output__printString(Memory__getString(PlayHelper___str3));
	screenBase = screenBase+(2*increment);
	PlayHelper__drawRightArrow(screenBase);
	Output__moveCursor(10, 1);
	Output__printString(Memory__getString(PlayHelper___str4));
	Output__moveCursor(21, 0);
	Output__printString(Memory__getString(PlayHelper___str5));
	Output__moveCursor(22, 0);
	Output__printString(Memory__getString(PlayHelper___str6));
	Output__moveCursor(20, 25);
	Output__printString(Memory__getString(PlayHelper___str7));
	PlayHelper__drawRightLine();
	return 0;
}
var PlayHelper__drawUpArrow(var base) {
	Memory__poke(base+32, 60);
	Memory__poke(base+64, 126);
	Memory__poke(base+96, 255);
	Memory__poke(base+128, 60);
	Memory__poke(base+160, 60);
	Memory__poke(base+192, 60);
	Memory__poke(base+224, 60);
	Memory__poke(base+256, 60);
	Memory__poke(base+288, 60);
	Memory__poke(base+320, 60);
	return 0;
}
var PlayHelper__drawDownArrow(var base) {
	Memory__poke(base+32, 60);
	Memory__poke(base+64, 60);
	Memory__poke(base+96, 60);
	Memory__poke(base+128, 60);
	Memory__poke(base+160, 60);
	Memory__poke(base+192, 60);
	Memory__poke(base+224, 255);
	Memory__poke(base+256, 126);
	Memory__poke(base+288, 60);
	Memory__poke(base+320, 24);
	return 0;
}
var PlayHelper__drawRightArrow(var base) {
	Memory__poke(base+64, 32);
	Memory__poke(base+96, 127);
	Memory__poke(base+128, 255);
	Memory__poke(base+160, 255);
	Memory__poke(base+192, 127);
	Memory__poke(base+224, 32);
	return 0;
}
var PlayHelper__drawLeftArrow(var base) {
	Memory__poke(base+64, 4);
	Memory__poke(base+96, 254);
	Memory__poke(base+128, 255);
	Memory__poke(base+160, 255);
	Memory__poke(base+192, 254);
	Memory__poke(base+224, 4);
	return 0;
}
var PlayHelper__drawRightLine() {
	var lineIndex = 0;
	...
	lineIndex = 0;
	position = 16384+5;
	while (-1==((((lineIndex)<(256))?-1:0))) {
		Memory__poke(position, -32767);
...
#ifdef JACK_IMPLEMENTATION
#define player__ (__this+0)
#define maze__ (__this+1)
#define blockWidth__ (__this+2)
#define blockHeight__ (__this+3)
#define xCount__ (__this+4)
#define yCount__ (__this+5)
#define drawPadding__ (__this+6)
#define baseX__ (__this+7)
#define baseY__ (__this+8)
var PlayerController__new(var newPlayer, var newMaze, var xOrigin, var yOrigin, var bw, var bh, var xc, var yc, var pad) {
	var __this;
	var x = 0;
	var y = 0;
	__this = Memory__alloc(9);
	__poke(player__, newPlayer);
	__poke(maze__, newMaze);
	__poke(blockWidth__, bw);
	__poke(blockHeight__, bh);
	__poke(xCount__, xc);
	__poke(yCount__, yc);
	__poke(drawPadding__, pad);
	__poke(baseX__, xOrigin);
	__poke(baseY__, yOrigin);
	x = Player__getX(__peek(player__));
	y = Player__getY(__peek(player__));
	Screen__setColor(-1);
	Screen__drawRectangle(__peek(baseX__)+(x*__peek(blockWidth__))+__peek(drawPadding__), __peek(baseY__)+(y*__peek(blockHeight__))+__peek(drawPadding__), __peek(baseX__)+((x+1)*__peek(blockWidth__))-__peek(drawPadding__), __peek(baseY__)+((y+1)*__peek(blockHeight__))-__peek(drawPadding__));
	return __this;
}
var PlayerController__loop(var __this) {
	var direction = 0;
	...
	x = Player__getX(__peek(player__));
	y = Player__getY(__peek(player__));
	while (-1==((((((~((((x)==((__peek(xCount__)-1)))?-1:0)))|(~((((y)==(0))?-1:0)))))==(-1))?-1:0))) {
		while (-1==(((((key)<(130))?-1:0))|((((key)>(133))?-1:0)))) {
		...
		if (0!=(PlayerController__canPlayerGo(__this, direction))) {
			x = Player__getX(__peek(player__));
			y = Player__getY(__peek(player__));
			Screen__setColor(0);
			Screen__drawRectangle(__peek(baseX__)+(x*__peek(blockWidth__))+__peek(drawPadding__), __peek(baseY__)+(y*__peek(blockHeight__))+__peek(drawPadding__), __peek(baseX__)+((x+1)*__peek(blockWidth__))-__peek(drawPadding__), __peek(baseY__)+((y+1)*__peek(blockHeight__))-__peek(drawPadding__));
			Player__go(__peek(player__), direction);
			x = Player__getX(__peek(player__));
			y = Player__getY(__peek(player__));
			Screen__setColor(-1);
			Screen__drawRectangle(__peek(baseX__)+(x*__peek(blockWidth__))+__peek(drawPadding__), __peek(baseY__)+(y*__peek(blockHeight__))+__peek(drawPadding__), __peek(baseX__)+((x+1)*__peek(blockWidth__))-__peek(drawPadding__), __peek(baseY__)+((y+1)*__peek(blockHeight__))-__peek(drawPadding__));
...
var PlayerController__canPlayerGo(var __this, var direction) {
	...
	x = Player__getX(__peek(player__));
	y = Player__getY(__peek(player__));
	tempArr = __peek(__peek(maze__)+y);
	val = __peek(tempArr+x);
	if (0!=(((((val&direction))==(direction))?-1:0))) {
		return -1;
	}
...
var PseudoRand__next(var __this, var mod) {
	...
	term = __peek(cur__);
	term = (term*17)+5;
	term = PseudoRand__modula(__this, term, 31);
	__poke(cur__, term);
	while (-1==((((term)>(mod))?-1:0))) {
		term = term-mod;
	}
	return term;
}
var PseudoRand__modula(var __this, var number, var mod) {
	while (-1==((((number)>(mod))?-1:0))) {
		number = number-mod;
	}
	if (0!=((((number)==(mod))?-1:0))) {
		number = 0;
...
var MazeDrawer__drawMaze(var maze, var originX, var originY, var blockWidth, var blockHeight, var xCount, var yCount) {
	...
	val = 0;
	Screen__setColor(-1);
	Screen__drawRectangle(originX, originY, originX+(xCount*blockWidth), originY+(yCount*blockHeight));
	Screen__setColor(0);
	Screen__drawRectangle(originX+1, originY+1, originX+(xCount*blockWidth)-1, originY+(yCount*blockHeight)-1);
	while (-1==((((y)<(yCount))?-1:0))) {
		tempArr = __peek(maze+y);
		x = 0;
		while (-1==((((x)<(xCount))?-1:0))) {
			val = __peek(tempArr+x);
			if (0!=(((((val&4))==(0))?-1:0))) {
				MazeDrawer__drawEastWall(x, y, originX, originY, blockWidth, blockHeight);
			}

			if (0!=(((((val&2))==(0))?-1:0))) {
				MazeDrawer__drawSouthWall(x, y, originX, originY, blockWidth, blockHeight);
			}
...
var MazeDrawer__drawEastWall(var x, var y, var originX, var originY, var bw, var bh) {
	Screen__setColor(-1);
	Screen__drawLine(originX+bw+(bw*x), originY+(y*bh), originX+bw+(bw*x), originY+bh+(y*bh));
	return 0;
}
var MazeDrawer__drawSouthWall(var x, var y, var originX, var originY, var bw, var bh) {
	Screen__setColor(-1);
	Screen__drawLine(originX+(bw*x), originY+bh+(y*bh), originX+(bw*x)+bw, originY+bh+(y*bh));
	return 0;
}
#endif
...
#ifdef JACK_IMPLEMENTATION
var MazeGenerator__generate(var grid, var rng, var mazeWidth, var mazeHeight) {
	MazeGenerator__carvePassageFrom(0, 0, grid, rng, mazeWidth, mazeHeight);
	return 0;
}
var MazeGenerator__carvePassageFrom(var cx, var cy, var grid, var rng, var mazeWidth, var mazeHeight) {
	var nextRand = 0;
	...
	temp = 0;
	while (-1==((((temp)<(4))?-1:0))) {
		nextRand = PseudoRand__next(rng, 4);
		pw = ExtendedMath__pow(2, nextRand);
		while (-1==((((taken&pw)==(pw))?-1:0))) {
			nextRand = nextRand+1;
			if (0!=(((((((((((nextRand)>(4))?-1:0)|nextRand)==(4))?-1:0)))==(-1))?-1:0))) {
				nextRand = nextRand-4;
			}
		...
		if (0!=((((((((ny)>(-1))?-1:0))&((((ny)<(mazeHeight))?-1:0))&((((nx)>(-1))?-1:0))&((((nx)<(mazeWidth))?-1:0)))==(-1))?-1:0))) {
			tempArr = __peek(grid+ny);
			if (0!=((((__peek(tempArr+nx))==(0))?-1:0))) {
				__poke(tempArr+nx, __peek(tempArr+nx)|opposite);
				tempArr = __peek(grid+cy);
				__poke(tempArr+cx, __peek(tempArr+cx)|direction);
				MazeGenerator__carvePassageFrom(nx, ny, grid, rng, mazeWidth, mazeHeight);
			}
...
#ifdef JACK_IMPLEMENTATION
var Main___str0[] = {89,79,85,32,87,79,78,33,0};
var Main__main() {
	var startRand = 0;
	var temp = 0;
	var temp2 = 0;
	...
	bh = 24;
	padding = 6;
	PlayHelper__drawPlayHelper();
	startRand = Main__getFirstKeyPressed();
	rng = PseudoRand__def(startRand);
	Screen__setColor(0);
	Screen__drawRectangle(96, 0, 511, 255);
	maze = Array__new(mazeHeight);
	temp = 0;
	while (-1==((((temp)<(mazeHeight))?-1:0))) {
		__poke(maze+temp, Array__new(mazeWidth));
		temp2 = 0;
		while (-1==((((temp2)<(mazeWidth))?-1:0))) {
			tempArr = __peek(maze+temp);
			__poke(tempArr+temp2, 0);
			temp2 = temp2+1;
		}
		temp = temp+1;
	}
	MazeGenerator__generate(maze, rng, mazeWidth, mazeHeight);
	MazeDrawer__drawMaze(maze, xOrigin, yOrigin, bw, bh, mazeWidth, mazeHeight);
	player = Player__new(0, 0);
	controller = PlayerController__new(player, maze, xOrigin, yOrigin, bw, bh, mazeWidth, mazeHeight, padding);
	Main__drawEndPoint(xOrigin, yOrigin, bw, bh, mazeWidth, mazeHeight, 8);
	PlayerController__loop(controller);
	Screen__setColor(0);
	Screen__drawRectangle(0, 0, 511, 255);
	Screen__setColor(-1);
	Output__moveCursor(11, 28);
	Output__printString(Memory__getString(Main___str0));
	return 0;
}
var Main__drawEndPoint(var baseX, var baseY, var bw, var bh, var xc, var yc, var rad) {
	Screen__setColor(-1);
	Screen__drawCircle(baseX+((xc-1)*bw)+(bw/2), baseY+(bh/2), rad);
	return 0;
}
var Main__getFirstKeyPressed() {
	var key = 0;
